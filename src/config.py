...
class Settings:
    ...
    MODEL_NAME: str = field(
        default_factory=lambda: os.getenv("MODEL_NAME", "ChurnOps-Model")
    )

    # File System & Artifact Paths
    DATA_RAW_DIR: str = field(
        default_factory=lambda: os.getenv("DATA_RAW_DIR", "data/raw")
    )
    DEFAULT_DATA_PATH: str = field(
        default_factory=lambda: os.getenv("DEFAULT_DATA_PATH", "data/raw/telco_churn.csv")
    )
    UPLOAD_DATA_PATH: str = field(
        default_factory=lambda: os.getenv("UPLOAD_DATA_PATH", "data/raw/user_upload.csv")
    )
    MODELS_DIR: str = field(
        default_factory=lambda: os.getenv("MODELS_DIR", "models")
    )
    BEST_MODEL_PATH: str = field(
        default_factory=lambda: os.getenv("BEST_MODEL_PATH", "models/best_model.joblib")
    )
    PREPROCESSOR_PATH: str = field(
        default_factory=lambda: os.getenv("PREPROCESSOR_PATH", "models/preprocessor.joblib")
    )
    ...
    REPORTS_PLOTS_DIR: str = field(
        default_factory=lambda: os.getenv("REPORTS_PLOTS_DIR", "reports/plots")
    )
    ...
    FAST_MODE_TRIALS: int = field(
        default_factory=lambda: int(os.getenv("FAST_MODE_TRIALS", "5"))
    )
    FULL_MODE_TRIALS: int = field(
        default_factory=lambda: int(os.getenv("FULL_MODE_TRIALS", "15"))
    )

    # Business Cost Optimization Weights
    COST_FN: float = field(
        default_factory=lambda: float(os.getenv("COST_FN", "500.0"))
    )
    COST_FP: float = field(
        default_factory=lambda: float(os.getenv("COST_FP", "50.0"))
    )

    # Monitoring & Drift Thresholds
    DRIFT_P_VALUE_THRESHOLD: float = field(
        default_factory=lambda: float(os.getenv("DRIFT_P_VALUE_THRESHOLD", "0.05"))
    )
    ...
    RATE_LIMIT_PER_MINUTE: int = field(
        default_factory=lambda: int(os.getenv("RATE_LIMIT_PER_MINUTE", "100"))
    )
