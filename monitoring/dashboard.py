"""
Streamlit Monitoring Dashboard for ChurnOps MLOps Pipeline.
"""
...
st.set_page_config(
    page_title="ChurnOps | Real-Time MLOps Dashboard",
    page_icon="⚡",
    layout="wide",
...
st.title("⚡ ChurnOps Production Monitoring Dashboard")
...
if df_logs.empty:
    st.warning("⚠️ No prediction logs found in SQLite database (`monitoring/predictions.db`). Submit prediction requests via FastAPI `/predict` endpoint to populate live metrics.")
    st.stop()
...
with tab1:
    ...
    with col_chart1:
        ...
        df_ts = df_logs.set_index("timestamp").resample("1h").size().reset_name if False else df_logs.set_index("timestamp").resample("1h")["id"].count().reset_index()
    ...
    st.dataframe(
        df_logs[["timestamp", "customerID" if "customerID" in df_logs else "id", "Contract", "InternetService", "MonthlyCharges", "churn_prediction", "churn_probability", "model_version"]].head(10),
...
with tab2:
    ...
    if drift_report.get("status") == "success":
        ...
        if alert:
            st.error("🚨 **CRITICAL DRIFT ALERT TRIGGERED** (PSI > 0.25 on one or more features). Model retraining recommended!")
        elif max_psi > 0.10:
            st.warning("⚠️ **MODERATE DATA DRIFT DETECTED** (0.10 <= PSI < 0.25). Monitor closely.")
        else:
            st.success("✅ **DATA DISTRIBUTION STABLE** (PSI < 0.10 across all monitored features).")
        ...
        selected_feat = st.selectbox("Select Feature to Inspect:", ["MonthlyCharges", "tenure", "TotalCharges", "Contract", "InternetService"])
